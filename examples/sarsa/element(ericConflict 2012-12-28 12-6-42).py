...
#	max_episode_times = 1000000	# max times of episode
	...
	def updateQ(self, algorithm = "Sarsa"):
		...
		if  algorithm == "Sarsa":
			...
			self.q_map[self.current_state][next_state] = (1 - 0.4) * self.q_map[self.current_state][next_state] +  0.4 * (self.r_map[self.current_state][next_state] + 0.8 * tmp[random_choice])			
		elif algorithm == "Q":
			# Q-learning
			self.q_map[self.current_state][next_state] = (1 - 0.4) * self.q_map[self.current_state][next_state] +  0.4 * (self.r_map[self.current_state][next_state] + 0.8 * max(tmp))			
		else:
			print "Have not set updateQ algorithm"
			return
