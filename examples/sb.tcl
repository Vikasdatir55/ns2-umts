...
$ns at 0.0 "$WLAN_UE0 setdest 150.0 100.0 50.0"
...
$ns at 0.0 "$WLAN_UE1 setdest 50.0 100.0 50.0"
...
$UE1 add-interface-node $WLAN_UE1
...
# create a TCP agent and attach it to multi-interface node puts "##############################################################" puts "***************Generating traffic: using TcpApp***************"
puts ""
...
$UE0 connect-agent $udp(0) $null(0) $UMTS_UE0
...
$UE1 attach-agent $udp(1) $UMTS_UE1                   ;# new command: the interface is used for sending
set null(1) [new Agent/Null] 
$ns attach-agent $CN_host0 $null(1)
$UE1 connect-agent $udp(1) $null(1) $UMTS_UE1
set cbr(1) [new Application/Traffic/CBR]
$cbr(1) attach-agent $udp(1)
$cbr(1) set type_ CBR 
$cbr(1) set packet_size_ 660
$cbr(1) set rate_ 2mb
...
$ns attach-hsdsch $UMTS_UE0 $udp_(0)
$ns attach-hsdsch $UMTS_UE1 $udp_(1)
...
proc wifiToumts {} {
	puts ""
	puts " time to wifi handove to umts"
	global udp cbr UE0 UE1 UMTS_UE0 UMTS_UE1 udp_ null ns
	$ns trace-annotate "Redirecting traffic to umts"
	$ns attach-agent $UMTS_UE0 $udp(0)
	$ns connect $udp(0) $null(0)
	$ns attach-agent $UMTS_UE1 $udp(1)
	$ns connect $udp(1) $null(1)


    #$UE0 attach-agent $udp_(0) $UMTS_UE0 						;# the interface is used for sending
    #$UE1 attach-agent $udp_(1) $UMTS_UE1
	#$cbr(0) attach-agent $udp_(0)
	#$cbr(1) stop
	#set cbr(2) [new Application/Traffic/CBR]
	#set cbr(3) [new Application/Traffic/CBR]
	#$cbr(2) attach-agent $udp_(1)
	#$cbr(3) attach-agent $udp_(1)
    #$UE0 connect-agent $udp_(0) $null(0) $UMTS_UE0 				;# the interface is used for receiving
    #$UE1 connect-agent $udp_(1) $null(1) $UMTS_UE1

	#$cbr(2) start
	#$ns at [expr [$ns now]+0.01] "$cbr(3) start"
}
